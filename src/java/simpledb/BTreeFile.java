...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
			Field f)
				...
				while (pid.pgcateg() == BTreePageId.INTERNAL) {
						...
						Iterator<BTreeEntry> it = bp.iterator();
						...
						while (it.hasNext()) {
								...
								if (f.compare(Predicate.Op.GREATER_THAN, e.getKey())) {
										te = e;
								} else if (f.compare(Predicate.Op.EQUALS, e.getKey())) {
										te = e;
										break;
								} else {
										if (!it.hasNext() || te == null) {
												te = e;
										}
										break;
								}
						...
						if (te == null) {
								throw new DbException("Internal logical error or zero size iterator");
						} else if (f == null) {
								pid = te.getLeftChild();
						} else if (f.compare(Predicate.Op.GREATER_THAN, te.getKey())) {
							 	pid = te.getRightChild();
						} else {
								pid = te.getLeftChild();
						}
				}
				...
				if (pid.pgcateg() != BTreePageId.LEAF) {
						throw new DbException("find wrong type page(should be leaf)");
				}

				BTreeLeafPage bp = (BTreeLeafPage)this.getPage(tid, dirtypages, pid, perm);
				return bp;
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// tuple with the given key field should be inserted.
				...
				if (trueTuple == null) {
						throw new DbException("end up with empty page");
				}

				BTreeLeafPage choosen = page;

				if (field.compare(Predicate.Op.GREATER_THAN, trueTuple.getField(this.keyField))) {
						choosen = newLeaf;
				}

				BTreeLeafPage sibling = null;
				if (page.getRightSiblingId() == null) {
						sibling = null;
				} else {
						sibling = (BTreeLeafPage)getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
						sibling.setLeftSiblingId(newLeaf.getId());
				}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// should be inserted.
			...
			BTreeEntry push = it.next();
			page.deleteKeyAndRightChild(push);
			BTreeInternalPage choosen = page;

			if (field.compare(Predicate.Op.GREATER_THAN, push.getKey())) {
					choosen = newInternal;
			}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// the corresponding parent entry.
			...
			int siblingNum = sibling.getNumTuples();
			int evenNum = (pageNum + siblingNum) / 2;
			int givenNum = evenNum - pageNum;
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// pointers of all children in the entries that were moved.
			...
			int siblingNum = leftSibling.getNumEntries();
			int evenNum = (pageNum + siblingNum) / 2;
			int givenNum = evenNum - pageNum;

			Iterator<BTreeEntry> it = leftSibling.reverseIterator();
			Field f = parentEntry.getKey();
			BTreeEntry pe = new BTreeEntry(f, leftSibling.reverseIterator().next().getRightChild(), page.iterator().next().getLeftChild());
			page.insertEntry(pe);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// pointers of all children in the entries that were moved.
      int pageNum = page.getNumEntries();
			int siblingNum = rightSibling.getNumEntries();
			int evenNum = (pageNum + siblingNum) / 2;
			int givenNum = evenNum - pageNum;

			Iterator<BTreeEntry> it = rightSibling.iterator();
			Field field = parentEntry.getKey();
			BTreeEntry pe = new BTreeEntry(field, page.reverseIterator().next().getRightChild(), rightSibling.iterator().next().getLeftChild());
			page.insertEntry(pe);
			for (int i=0; i<givenNum-1; i++) {
					BTreeEntry e = it.next();
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// deleteParentEntry() will be useful here
			...
			if (rightPage.getRightSiblingId() != null) {
					BTreeLeafPage rRightPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
